...
it('should test the test', (done) => {
  Promise.resolve()
    .then(() => HttpHook.wireHttpHook()) // hook the hook
    .then(() => doSomethingToPerformNetworkRequest())
    .then(() => waitForChange())
    .then(() => {
      return HttpHook
        .getLoggedData('url-substring')
        .then((data) => console.log(data.requestId)) // you can do whatever you like here. expect conditions, or just log the data or any error
    })
    .then(() => HttpHook.unwireHttpHook())
    .then(() => done())
